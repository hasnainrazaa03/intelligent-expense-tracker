...
- A **primitive component library** now lives in `client/src/components/ui/` — `Button`/`IconButton` (variant + size), `Card`, `Field` (`Input`/`Textarea`/`Select`/`Label` + shared `fieldClass`/`labelClass`), and a `Modal` shell that folds in the focus trap. 14 surfaces adopt them, deduping the repeated token-class stacks (the first pass applied token classes directly to ship the redesign faster; this follow-up extracted the reusables).
- The legacy neo tokens were **removed**: `tailwind.config.js` no longer carries `usc-*`/`ink`/`bone` colors or `shadow-neo*`, the unused `ThemeToggle`/`CurrencyToggle` components were deleted, and the dead `.font-loud`/`.graph-grid` blocks were dropped from `index.css`.
- Starfield ships **static** (not animated) for performance: a continuously animated full-viewport layer behind ~10 `backdrop-blur` panels forced a per-frame re-blur. Blur is now limited to the fixed chrome via `.glass-blur`. Measured tab-switch is ~30–90 ms; the heavy libs (recharts, jsPDF, html2canvas, tesseract) are lazy-loaded chunks.
